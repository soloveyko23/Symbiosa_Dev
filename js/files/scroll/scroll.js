...
export function digitsCounter() {
  // Обнуление
  const elements = document.querySelectorAll("[data-digits-counter]");
  if (elements.length) {
  ...
  function digitsCountersAnimate(digitsCounter) {
    ...
    const startValue = parseFloat(digitsCounter.dataset.digitsCounter.replace(/[^\d.]/g, ''));
    ...
    const step = (timestamp) => {
        ...
        // Заменяем &nbsp; на точку
        const formattedValue = currentValue.toLocaleString().replace(/&nbsp;/g, '.');

        digitsCounter.innerHTML = formattedValue;
...
// Функція для додавання роздільника тисяч
