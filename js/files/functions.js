...
export function spollers() {
    ...
    function setSpollerAction(e) {
      ...
      // Проверка, является ли элемент (или его предки) частью класса 'checkbox'
      if (el.closest('.checkbox')) {
          return; // Прекращаем действие, если элемент является частью 'checkbox'
      }
...
export function showMore() {
	window.addEventListener("load", function (e) {
		...
		function initItem(showMoreBlock, matchMedia = false) {
          ...
          showMoreContent.classList.add('showmore-not-active')
				}
			} else {
				_slideDown(showMoreContent, 0, hiddenHeight);
				showMoreButton.hidden = true;
